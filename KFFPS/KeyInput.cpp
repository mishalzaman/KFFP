...
void KeyInput::update(float deltaTime)
{
	while (SDL_PollEvent(&this->event)) {
		...
		case SDL_KEYDOWN:
			...
			// esc
			if (event.key.keysym.sym == SDLK_ESCAPE) { this->esc = true; }
			break;
		case SDL_KEYUP:
			...
			if (event.key.keysym.sym == SDLK_ESCAPE) { this->quit = false; }
			// esc
			if (event.key.keysym.sym == SDLK_ESCAPE) { this->esc = false; }
			break;
...
bool KeyInput::isEsc() { return this->esc; }
